...
const Bora = {
  ...
  minhasLojas() { return this.api('/api/rede/lojas'); },
  trocarLoja(lojaId) { return this.api('/auth/trocar-loja', { method: 'POST', body: JSON.stringify({ lojaId }) }); },
  balancete(inicio, fim) { const q = []; if (inicio) q.push('inicio=' + inicio); if (fim) q.push('fim=' + fim); return this.api('/api/rede/balancete' + (q.length ? '?' + q.join('&') : '')); },
  analiseCanais(inicio, fim) { const q = []; if (inicio) q.push('inicio=' + inicio); if (fim) q.push('fim=' + fim); return this.api('/api/analise/canais' + (q.length ? '?' + q.join('&') : '')); },
  analiseHorario(inicio, fim) { const q = []; if (inicio) q.push('inicio=' + inicio); if (fim) q.push('fim=' + fim); return this.api('/api/analise/horario' + (q.length ? '?' + q.join('&') : '')); },
  analiseTempos(inicio, fim) { const q = []; if (inicio) q.push('inicio=' + inicio); if (fim) q.push('fim=' + fim); return this.api('/api/analise/tempos' + (q.length ? '?' + q.join('&') : '')); },
  onboarding() { return this.api('/api/onboarding'); },
  recebimento() { return this.api('/api/recebimento'); },
  ...
  acertoPrevia(inicio, fim) { return this.api('/api/acertos/previa?inicio=' + inicio + '&fim=' + fim); },
  fazerAcerto(body) { return this.api('/api/acertos', { method: 'POST', body: JSON.stringify(body) }); },
  acertoHistorico() { return this.api('/api/acertos/historico'); },
